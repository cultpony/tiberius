...
pub(crate) async fn run_job(current_job: CurrentJob, sctx: SharedCtx) -> TiberiusResult<()> {
    ...
    let tx = sentry::start_transaction(sentry::TransactionContext::new("generate_thumbnails", "queue.task"));
...
async fn tx_run_job(current_job: CurrentJob, sctx: SharedCtx) -> TiberiusResult<()> {
    ...
    let progress: GenerateThumbnailConfig = current_job.json()?.expect("job requires configuration copy");
    ...
    let img = sctx.config.image_base().join(img.image.expect("can't generate thumbs for unstored images"));

    let img = std::fs::File::open(img)?;
    let img = std::io::BufReader::new(img);

    let img = tokio::task::spawn_blocking(move || {
        image::load(img, format)
    }).await??;
    ...
    let (large, medium, small, thumb) = tokio::join!(
        do_make_thumb(ImageThumbType::Large, configuration.image_base().join("thumbs").join(thumbs.large.path().trim_start_matches("/img/")), format, img.clone()),
        do_make_thumb(ImageThumbType::Medium, configuration.image_base().join("thumbs").join(thumbs.medium.path().trim_start_matches("/img/")), format, img.clone()),
        do_make_thumb(ImageThumbType::Small, configuration.image_base().join("thumbs").join(thumbs.small.path().trim_start_matches("/img/")), format, img.clone()),
        do_make_thumb(ImageThumbType::Thumb, configuration.image_base().join("thumbs").join(thumbs.thumb.path().trim_start_matches("/img/")), format, img.clone()),
    );
