...
pub async fn server_error() -> TiberiusResponse<()> {
    let content = error_page(&TiberiusError::Other(format!("Sorry for that, we encountered an issue with your request."))).await.into_string();
    TiberiusResponse::Html(HtmlResponse{
        content,
    })
}