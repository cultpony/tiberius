...
pub async fn list_tags() -> TiberiusResult<HtmlResponse> {
    Ok(HtmlResponse{ content: todo_page("list_tags").await?.into_string() })
}

#[get("/tags/<tag_id>")]
pub async fn show_tag(tag_id: i64) -> TiberiusResult<HtmlResponse> {
    Ok(HtmlResponse{ content: todo_page("show_tags").await?.into_string() })
}
...
pub async fn edit_tag(tag_id: i64) -> TiberiusResult<HtmlResponse> {
    Ok(HtmlResponse{ content: todo_page("edit_tags").await?.into_string() })
}

#[get("/tags/<tag_id>/changes")]
pub async fn tag_changes(tag_id: i64) -> TiberiusResult<HtmlResponse> {
    Ok(HtmlResponse{ content: todo_page("tag_tags").await?.into_string() })
}

#[get("/tags/<tag_id>/usage")]
pub async fn usage(tag_id: i64) -> TiberiusResult<HtmlResponse> {
    Ok(HtmlResponse{ content: todo_page("usage_tags").await?.into_string() })
}

#[post("/tags/<tag_id>/reindex")]
pub async fn reindex(tag_id: i64) -> TiberiusResult<TiberiusResponse<()>> {
    Ok(TiberiusResponse::Html(HtmlResponse{ content: todo_page("usage_tags").await?.into_string() }))
}
...
pub async fn autocomplete(state: &State<TiberiusState>, term: String) -> TiberiusResult<JsonResponse> {
    ...
    log::debug!("Autocomplete found {} tags for {:?}", count, term);
    let autocompleted_tags: Vec<AutocompleteResponse> = autocompleted_tags.1.iter().map(|tag| {
