        ...
        Ok(match self {
            ...
            Query::Tag { n, v } => {
                ...
                Box::new(TermQuery::new(
                    tantivy::Term::from_field_text(schema.get_field("tag").unwrap(), &v),
...
mod test {
    ...
    fn test_death_query_tantivy() -> anyhow::Result<()> {
        ...
        schema_builder.add_text_field("tags", tantivy::schema::STRING);
    ...
    fn test_attrcomp_query_tantivy() -> anyhow::Result<()> {
        let query = "width.gte:1024,aspect_ratio.lte:2.0,created.lte:3 days ago";
        let fs = crate::tokenizer::parse(query);
        let exp = r#"BooleanQuery { subqueries: [(Must, BooleanQuery { subqueries: [(Must, RangeQuery { field: Field(0), value_type: I64, left_bound: Unbounded, right_bound: Included([128, 0, 0, 0, 0, 0, 4, 0]) }), (Must, RangeQuery { field: Field(1), value_type: F64, left_bound: Included([192, 0, 0, 0, 0, 0, 0, 0]), right_bound: Unbounded })] }), (Must, RangeQuery { field: Field(2), value_type: Date, left_bound: Unbounded, right_bound: Included([128, 0, 0, 0, 97, 17, 129, 174]) })] }"#;
        let q = Query::from_foldstate(0, fs);
        let q = match q {
            Ok(v) => v,
            Err((q, e)) => anyhow::bail!("Error: {}: Got so far: {}", e, q),
